...
- **Build**: Triggered on every push and pull request to `main`, `master`, or `develop`.
  - Compiles the project using JDK 25.
  - Runs unit tests.
  - Performs compatibility tests on Paper servers (versions 1.20.4, 1.21.1, 1.21.4, 26.1.2).
...
Currently, the plugin target is **Minecraft 1.21.4** (tested up to **26.1.2**) and requires **Java 21+** (Java 25 recommended for newer server versions). 
