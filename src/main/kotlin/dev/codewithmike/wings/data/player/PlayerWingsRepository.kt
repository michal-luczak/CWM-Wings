...
object PlayerWingsRepository {
    ...
    fun get(playerUuid: UUID): PlayerWingsDto? = transaction {
        PlayerWingsTable
            .selectAll()
    ...
    fun findAll(): Set<PlayerWingsDto> = transaction {
        PlayerWingsTable.selectAll()
            .map { PlayerWingsDto(
    ...
    fun grant(playerUuid: UUID, wingsDefinitionId: String) = transaction {
    ...
    fun revoke(playerUuid: UUID) = transaction {
        PlayerWingsTable.deleteWhere {
            this.playerUuid eq playerUuid.toString()
        }
    }

    fun turnOnWings(playerUuid: UUID) = transaction {
    ...
    fun turnOffWings(playerUuid: UUID) = transaction {
        PlayerWingsTable.update(
            { PlayerWingsTable.playerUuid eq playerUuid.toString() }
        ) {
            it[enabled] = false
